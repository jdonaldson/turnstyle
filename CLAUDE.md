...
- **Full 27-task BBH v9: 80.67%** (2026-04-24): +1.76pp over v8, driven entirely by the hyperbaton solver. Synthesized — full v9 27-task run hangs at hyperbaton in full-eval context (see Known Issues), so result was composed from v8 unchanged tasks + solo hyperbaton run. Result: `swollm/results/v9/bbh_full.json`.
...
- **MPS cumulative-state crash in `swollm evaluate`** (2026-04-24): full 27-task eval reproducibly hangs at hyperbaton's bypass phase after 7 prior tasks have run. Process exits cleanly with a `resource_tracker` semaphore-leak warning and no `bypass:` line; no Python traceback. Solo hyperbaton runs in 24.7s. `torch.mps.empty_cache()` between tasks did not help. Workaround: run new/changed tasks solo and synthesize aggregates from prior full-eval. Open root cause — likely cumulative MPS allocator state interacting with the per-adjective L8 forward passes in `solve_hyperbaton`. Reproducer: `swollm evaluate` with all 27 tasks; crashes once it reaches hyperbaton.
