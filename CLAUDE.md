...
- **Relational transcription pipeline** (2026-04-06): no-regex 5-task experiment with Qwen 1.5B. L1 boundary probe (100% in-distribution) replaces LLM segmenter. Flexible accumulator (structural typing by field presence, not field names) + accumulator-side fallbacks. Suite average 75.8% → **93.4%** (+17.6pp). Per-task: navigate 100%, web_of_lies 98.8%, **tracking_shuffled 92.4%** (+31.2pp via 3-line accumulator patch), logical_deduction 76.0%, **object_counting 100%** (+35.6pp via 4-rule accumulator fix — recurse nested items, accept `type: object`, recover from `type: number` via segment text, segment-text fallback in compute). **Key lesson (confirmed twice)**: don't fight Qwen's learned output format — diagnose the failure modes, then fix the consumer. Two prompt-side fixes (list-output, pluralized container) both lost ≥10pp before the accumulator approach landed. See `experiments/` and memory `relational_transcription.md`.
