...
- **Relational transcription pipeline** (2026-04-06): no-regex 5-task experiment with Qwen 1.5B. L1 boundary probe (100% in-distribution) replaces LLM segmenter. Flexible accumulator (structural typing by field presence, not field names) + accumulator-side `person1` fallback. Suite average 75.8% → **86.5%** (+10.7pp). Per-task: navigate 100%, web_of_lies 98.8%, **tracking_shuffled 92.4%** (+31.2pp via 3-line accumulator patch), logical_deduction 76.0%, object_counting 64.4% (next target). **Key lesson**: don't fight Qwen's learned output format — fix the consumer. Two prompt-side fixes (list-output, pluralized container) both lost ≥10pp before the accumulator fix landed. See `experiments/accumulator_fix_experiment.py` and memory `relational_transcription.md`.
