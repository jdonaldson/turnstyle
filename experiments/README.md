...
| `purpose_discover.py` | What ARE the purpose categories? Unsupervised k-means on 61 gerunds → **they're VERB-semantic classes** (Levin/VerbNet-style): NMI 0.87/ARI 0.71 vs seeded classes @L13. So the "purpose" rung is the model's verb/event space (deverbal gerunds), peaking MID-stack L13 (deeper than scalar attribute frames L2–4). **(But the k=10 here was design-circular — see purpose_count.)** | 2026-06-22 |
| `verb_structure.py` | If verb space isn't clusters, what shape is it? Fit 6 verb-property axes (physicality/dynamism/force/volition/sociality/valence) — all extremes interpretable, semi-independent (\|cos\| 0.01–0.33). But they capture only **~14% of variance**; PR=51, flat PCA spectrum → a **high-D continuum**, not low-D. Top PC≈**volition** (0.73, the dominant gradient = the robust unsupervised split), PC2≈physicality/valence. Same low-variance-semantics pattern as color/concept-geometry. | 2026-06-22 |
| `purpose_count.py` | How many verb classes? **No clean discrete count.** Unbiased 131-gerund set @L13: silhouette flat+low (0.03–0.06, no peak), PR=52 (high-dim), only robust split is k=2 (expressive/involuntary bodily verbs vs volitional action). Verb semantics is high-dim + GRADED, not a small taxonomy — supervised it's decodable (impose any classes), unsupervised there's no N. Matches Levin/VerbNet fuzziness. frpr: corrects purpose_discover's circular k=10. | 2026-06-22 |
